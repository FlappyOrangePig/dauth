...
import com.cyberflow.dauthsdk.login.const.LoginConst.ACCESS_TOKEN
...
class DummyLogin internal constructor() : ILoginApi, IWalletApi by WalletHolder.walletApi {
    ...
    private suspend fun loginAuth(codeChallengeCode: String, didToken: String): String {
        val map = HashMap<String, String>()
        var code = ""
        map[LoginConst.USER_TYPE] = "10"
        map[LoginConst.CODE_CHALLENGE] = codeChallengeCode
        map[LoginConst.CODE_CHALLENGE_METHOD] = LoginConst.SIGN_METHOD
        val sign = SignUtils.sign(map)

        val body = AuthorizeParam(10, codeChallengeCode, LoginConst.SIGN_METHOD, sign)
    ...
    private fun getDAuthToken(
        ...
        val map = HashMap<String, String>()
        map[LoginConst.CODE_VERIFIER] = codeVerifier
        map[LoginConst.AUTH_CODE] = code
        val sign = SignUtils.sign(map)
    ...
    override fun createDAuthAccountApi(
        ...
        map[LoginConst.UUID] = "123456"
        map[LoginConst.IS_LOGIN] = "1"
        map[LoginConst.SEX] = "0"
        map[LoginConst.PASSWORD] = passWord
        map[LoginConst.CONFIRM_PASSWORD] = confirmPwd
        val sign = SignUtils.sign(map)
    ...
    override suspend fun loginByMobileOrEmailApi(
        ...
        var loginCode : Int = -1
        val map = HashMap<String, String>()
        map[LoginConst.USER_TYPE] = type.toString()
        map[LoginConst.VERIFY_CODE] = verifyCode
        if (type == 10) {
            map[LoginConst.ACCOUNT] = account
            val sign = SignUtils.sign(map)
    ...
    override fun logoutApi(openUid: String) {
        val map = HashMap<String, String>()
        map[LoginConst.OPEN_UID] = openUid
        val sign = SignUtils.sign(map)
    ...
    override fun sendPhoneVerifyCodeApi(phone: String, areaCode: String) {
        val map = HashMap<String, String>()
        map[LoginConst.PHONE] = phone
        map[LoginConst.PHONE_AREA_CODE] = areaCode
        val sign = SignUtils.sign(map)
